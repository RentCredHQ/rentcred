...
const { getDisputes, createDispute } = useDisputes()
  ...
  <div class="flex flex-col gap-6">
    <div class="flex items-start justify-between">
      <div class="flex flex-col gap-1">
        <h1 class="font-mono text-xl lg:text-2xl font-bold text-foreground">My Disputes</h1>
        <p class="font-sans text-sm text-muted-foreground">Challenge verification findings on your submissions</p>
      </div>
      <button
        v-if="!showForm"
      ...
      <div class="flex flex-col gap-4">
        <div class="flex flex-col gap-1.5">
          ...
          <select v-model="form.submissionId" class="w-full h-[42px] px-3.5 bg-background border border-border text-sm font-sans text-foreground focus:outline-none focus:border-primary transition-colors appearance-none">
            ...
            <option v-for="sub in submissions" :key="sub.id" :value="sub.id">{{ sub.tenantName }} — {{ sub.propertyAddress }}</option>
          </select>
        </div>
        <div class="flex flex-col gap-1.5">
    ...
    <div v-for="dispute in disputes" :key="dispute.id" class="bg-white border border-border rounded-xl p-5 lg:p-6 flex flex-col gap-3 shadow-sm">
      <div class="flex items-start justify-between">
        <div class="flex flex-col gap-0.5">
          ...
          <span class="font-sans text-[13px] text-muted-foreground">{{ dispute.submission?.tenantName }} &bull; {{ dispute.submission?.propertyAddress }}</span>
        ...
        <span class="inline-flex px-2.5 py-0.5 rounded-full text-[11px] font-semibold" :class="statusBadgeClasses(dispute.status)">
